...
function get20s(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    let sepYears = arr[i].years.split("-");
    if (
      sepYears[0] >= 1900 &&
      sepYears[0] <= 2000 &&
      sepYears[1] >= 1900 &&
...
function addArtist(arr) {
  const aboutMe = {
    ...
    nationality: "USA",
    bio:
      "Rika is a 23 year old student enrolled in Lambda School. She is doing her best to perform well on this sprint challenge",
  };
  arr.push(aboutMe);
  return arr;
}
